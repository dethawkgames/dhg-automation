/**
 * apply-preorder-templates.js
 *
 * The reverse of revert-expired-preorder-templates.js.
 *
 * Finds Shopify products (added in the last 6 months) that are:
 *   - tagged "preorder"
 *   - carry a "release-date-YYYY-MM-DD" tag that has NOT happened yet
 *   - NOT currently on the "preorder" theme template (templateSuffix)
 * and assigns them the "preorder" theme template.
 *
 * Usage:
 *   SHOPIFY_STORE=your-store.myshopify.com \
 *   SHOPIFY_CLIENT_ID=xxx \
 *   SHOPIFY_CLIENT_SECRET=xxx \
 *   node apply-preorder-templates.js
 *
 *   ...same env vars... node apply-preorder-templates.js --apply
 *
...
const UPDATE_MUTATION = `
  mutation ApplyPreorderTemplate($id: ID!) {
    productUpdate(input: { id: $id, templateSuffix: "${PREORDER_TEMPLATE_SUFFIX}" }) {
      product {
        id
        templateSuffix
      }
...
async function main() {
  ...
  const alreadyOnPreorderTemplate = [];
  ...
  for (const p of products) {
    ...
    const notYetReleased = releaseDate >= today;
    const onPreorderTemplate = p.templateSuffix === PREORDER_TEMPLATE_SUFFIX;

    if (!notYetReleased) {
      alreadyReleased.push({ ...p, releaseDate });
      continue;
    }

    if (onPreorderTemplate) {
      alreadyOnPreorderTemplate.push({ ...p, releaseDate });
      continue;
    }

    toApply.push({ ...p, releaseDate });
  }

  console.log(`--- Products to switch to the preorder template (${toApply.length}) ---`);
  for (const p of toApply) {
    console.log(`  ${p.title} — releases ${p.releaseDate} — currently: ${p.templateSuffix || '(default)'}`);
  }

  console.log(`\n--- Already on the preorder template, not yet released (${alreadyOnPreorderTemplate.length}) ---`);
  for (const p of alreadyOnPreorderTemplate) {
    console.log(`  ${p.title} — releases ${p.releaseDate}`);
  }

  console.log(`\n--- Release date already passed, left alone (${alreadyReleased.length}) ---`);
  for (const p of alreadyReleased) {
  ...
  if (!APPLY) {
    console.log(`\nDry run complete. Re-run with --apply to switch the ${toApply.length} product(s) listed above.`);
    return;
  }

  console.log(`\nApplying changes to ${toApply.length} product(s)...`);
  for (const p of toApply) {
    try {
      ...
      if (errors && errors.length) {
        ...
        console.log(`  OK: ${p.title} — template now: ${data.productUpdate.product.templateSuffix}`);
      }
    } catch (err) {
